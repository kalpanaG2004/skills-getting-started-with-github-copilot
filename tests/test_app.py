...
def test_signup_and_unregister():
    ...
    # Unregister
    response_unreg = client.delete(f"/activities/{activity_name}/unregister?email={email}")
    assert response_unreg.status_code == 200
    assert "Unregistered" in response_unreg.json()["message"]

    # Unregister again should fail
    response_unreg2 = client.delete(f"/activities/{activity_name}/unregister?email={email}")
